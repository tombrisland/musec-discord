...
func PlayCommand(inst *instance.Instance, param string) {
	...
	if isUrl(param) {
		...
		if len(matches) > index && index != -1 {
			...
			if err != nil {
				...
				if err != nil {
					...
					discord.TextMessage(sd, "Nothing found for playlist `%s`", param)
				}
		...
		if err != nil {
			discord.TextMessage(sd, "No results found for search `%s`", param)
...
func playPlaylist(inst *instance.Instance, playlist *service.SearchResult) {
	...
	for _, vr := range playlist.Videos {
		...
		go func() {
			...
			if err != nil {
				log.Printf("Failed to add a track from a playlist [%s]\n", playlist.Title)
			} else {
				stop := make(chan bool)
	...
	discord.TextMessage(sd, "Added `%d` tracks from `%s`, total `%d` in the queue", len(playlist.Videos), playlist.Title, size)
}
...
func playTrack(inst *instance.Instance, id string) error {
	...
	if err != nil {
		discord.TextMessage(sd, "No video found at id/URL `%s`", id)
	...
	if pos > 1 || inst.Skip != nil {
		discord.TextMessage(sd, "Adding `%s` as number `%d` in the queue", video.Title, pos)
	}
