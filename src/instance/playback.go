...
func (p *Instance) BeginPlayback() {
	...
	for {
		...
		if err != nil {
			discord.TextMessage(sd, "I wasn't able to play `%s`", track.Video.Title)

			continue
		}

		discord.TextMessage(sd, "Playing `%s`", track.Video.Title)

		// Play the stream until it ends or a stop message is received
		err = PlayAudioStream(cs.VoiceConnection, stream, track.Stop)

		if err != nil {
			discord.TextMessage(sd, "I had trouble while playing `%s`", track.Video.Title)
		}
...
func (p *Instance) leave() {
	...
	if p.Inactive() {
		log.Printf("Leaving voice channel [%s]\n", s.VoiceChannel.Name)

		// Leave the channel until re-activated
		err := s.VoiceConnection.Disconnect()

		discord.TextMessage(s.Server, "Left voice after `%ds` of inactivity", int(afkDelay.Seconds()))

		if err != nil {
			log.Printf("Failed to leave voice channel [%s]\n", s.VoiceChannel.Name)
		}
	}
}
